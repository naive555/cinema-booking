...
- Backend: Go + Gin
- Frontend: Vue 3 (built, served via nginx)
- Database: MongoDB
...
## Current phase: RESUBMISSION — COMPLETE, READY TO ZIP
The assignment was submitted once and did not land the job. A hardening plan
(`.claude/plans/plan.md`) was written and fully executed: Redis Streams PEL
reclaim via XAUTOCLAIM + dead-letter, an explicit seat-release endpoint, JWT
moved out of URLs (fragment redirect + WebSocket subprotocol auth), WebSocket
origin allowlist, OAuth cookie hardening, frontend reconnect backoff, and a
README pass. All phases are live-verified and committed on `dev`.

Before touching this repo, read `.claude/plans/plan.md` for what was changed and
why, and `README.md` §4/§5/§7 for the concurrency and trade-off rationale — the
docs are deliberately kept in sync with the code, so any code change here needs
the matching README edit.

DO NOT: real payment gateway, email delivery, multi-seat booking, Redis pub/sub
WS fanout (documented as a trade-off only), k8s/CI/TLS — all explicitly out of
scope. Do not reopen the locked architecture decisions above.

## Secrets
`.env` is gitignored and holds placeholders only; the original Google OAuth
client secret was rotated after it sat on disk during development. Never commit
real credentials, and re-scrub `.env` before producing a submission zip.